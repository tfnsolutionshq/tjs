...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .mp-hero__eyebrow {
            margin: 0 0 .35rem; font-size: .7rem; font-weight: 700; letter-spacing: .1em;
            text-transform: uppercase; color: var(--muted);
        }
        .mp-hero__title {
            margin: 0; font-family: 'Libre Baskerville', Georgia, serif;
            font-size: clamp(1.45rem, 2.6vw, 1.9rem); font-weight: 700; color: var(--ink);
            letter-spacing: -.02em; line-height: 1.2;
        }
        .mp-hero__sub { margin: .45rem 0 0; font-size: .92rem; color: var(--muted); line-height: 1.5; max-width: 40rem; }
        .mp-actions { display: flex; flex-wrap: wrap; gap: .5rem; margin-top: 1rem; }
        .mp-stats { display: grid; grid-template-columns: 1fr 1fr; gap: .75rem; margin-bottom: 1.15rem; }
        @media (min-width: 860px) { .mp-stats { grid-template-columns: repeat(4, minmax(0, 1fr)); } }
        .mp-stat {
            background: #fff; border: 1px solid var(--line); border-radius: .95rem; padding: .95rem 1rem;
            box-shadow: 0 8px 24px rgba(15,23,42,.035);
        }
        .mp-stat__label { margin: 0; font-size: .68rem; font-weight: 700; letter-spacing: .08em; text-transform: uppercase; color: var(--muted); }
        .mp-stat__value { margin: .35rem 0 0; font-size: 1.55rem; font-weight: 800; letter-spacing: -.03em; color: var(--ink); line-height: 1; }
        ...
        .mp-card__title { margin: 0; font-size: .98rem; font-weight: 800; color: var(--ink); letter-spacing: -.01em; }
        .mp-card__desc { margin: .3rem 0 0; font-size: .82rem; color: var(--muted); line-height: 1.45; }
        .mp-card__body { padding: 1rem 1.15rem 1.2rem; }
        .mp-tile {
            display: block; text-decoration: none; color: inherit;
            background: #fff; border: 1px solid var(--line); border-radius: 1rem; padding: 1.1rem 1.15rem;
            box-shadow: 0 8px 24px rgba(15,23,42,.035);
            transition: transform .15s ease, border-color .15s ease, box-shadow .15s ease;
        }
        .mp-tile:hover { transform: translateY(-2px); border-color: #93c5fd; box-shadow: 0 14px 30px rgba(15,23,42,.07); }
        .mp-tile__icon {
            width: 2.35rem; height: 2.35rem; border-radius: .7rem; display: inline-flex; align-items: center; justify-content: center;
            background: #e8f1fc; color: #1d4ed8; margin-bottom: .75rem;
        }
        .mp-tile__icon svg { width: 1.1rem; height: 1.1rem; }
        .mp-tile__title { margin: 0; font-size: 1.02rem; font-weight: 800; color: var(--ink); }
        .mp-tile__text { margin: .35rem 0 0; font-size: .84rem; color: var(--muted); line-height: 1.45; }
        .mp-row {
            display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: .65rem;
            padding: .85rem 0; border-bottom: 1px solid #f1f5f9; text-decoration: none; color: inherit;
        }
        .mp-row:last-child { border-bottom: 0; }
        .mp-row:hover .mp-row__title { color: #1d4ed8; }
        .mp-row__title { margin: 0; font-size: .92rem; font-weight: 750; color: var(--ink); transition: color .15s ease; }
        .mp-row__meta { margin: .2rem 0 0; font-size: .76rem; color: var(--muted); }
        .mp-empty { margin: 0; padding: .5rem 0; font-size: .88rem; color: var(--muted); }
        .mp-badge {
        ...
        .mp-btn:disabled { opacity: .72; cursor: wait; pointer-events: none; }
        .mp-btn-primary { background: var(--blue); color: #fff; box-shadow: 0 8px 18px rgba(47,125,225,.22); }
        .mp-btn-primary:hover { filter: brightness(1.05); }
        .mp-btn-secondary { background: #fff; color: var(--ink); border: 1px solid var(--line); }
        .mp-btn-secondary:hover { border-color: #cbd5e1; }
        .mp-field label { display: block; margin-bottom: .4rem; font-size: .78rem; font-weight: 700; color: #334155; }
        .mp-field .mp-hint { margin: .35rem 0 0; font-size: .72rem; color: var(--muted); line-height: 1.4; }
        .mp-field .mp-error { margin: .35rem 0 0; font-size: .75rem; font-weight: 600; color: #b91c1c; }
        ...
        .mp-textarea { min-height: 7rem; resize: vertical; line-height: 1.5; }
        .mp-input:focus, .mp-select:focus, .mp-textarea:focus {
            border-color: #93c5fd; box-shadow: 0 0 0 3px rgba(47,125,225,.14);
        }
        .mp-file {
            display: block; width: 100%; font-size: .82rem; color: var(--muted);
            padding: .75rem; border: 1px dashed #cbd5e1; border-radius: .7rem; background: #f8fafc;
        }
        .mp-spinner {
            width: .95rem; height: .95rem; border: 2px solid rgba(255,255,255,.35);
            border-top-color: #fff; border-radius: 999px; animation: mp-spin .7s linear infinite;
        }
        @keyframes mp-spin { to { transform: rotate(360deg); } }
        .mp-timeline { display: grid; gap: .55rem; }
        .mp-timeline__item { display: grid; grid-template-columns: .75rem 1fr; gap: .75rem; align-items: start; }
        .mp-timeline__dot {
            width: .75rem; height: .75rem; border-radius: 999px; background: #93c5fd; margin-top: .35rem;
            box-shadow: 0 0 0 4px #eff6ff;
        }
        .mp-flash {
            margin-bottom: 1rem; padding: .85rem 1rem; border-radius: .85rem;
            background: #ecfdf5; border: 1px solid #a7f3d0; color: #047857; font-size: .88rem; font-weight: 600;
        }
        .mp-narrow { max-width: 46rem; }
...
@endphp
    <div class="admin-shell">
        ...
        <aside class="admin-sidebar" :class="{ 'is-open': sidebarOpen }">
            ...
            <nav class="admin-nav" aria-label="Member portal">
                <p class="admin-nav__label">Overview</p>
                <a href="{{ route('dashboard') }}" @class(['is-active' => request()->routeIs('dashboard')]) @click="sidebarOpen = false">
                ...
                <a href="{{ route('memberships.index') }}" @class(['is-active' => request()->routeIs('memberships.*')]) @click="sidebarOpen = false">
                ...
                <a href="{{ route('profile.edit') }}" @class(['is-active' => request()->routeIs('profile.*')]) @click="sidebarOpen = false">
                ...
                <div class="admin-user">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="admin-user__role">
                            @if($mpUser?->isAdmin())
                                Platform admin
                            @elseif($showReviews && $mpManaged->isEmpty())
                                Reviewer
                            @elseif($mpManaged->isNotEmpty())
                                Journal team
                            @else
                                Member
                            @endif
        ...
        <div class="admin-main">
            <header class="admin-topbar">
                <div class="flex min-w-0 items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <h1 class="admin-topbar__title truncate">@yield('page_title', 'Portal')</h1>
